...
// Extrai todos os polos únicos do sistema (criados por gestores e consultores)
function getAllPolos(users) {
  const map = {};
  (users||[]).filter(u=>u.status==="aprovado"&&(u.role==="gestor"||u.role==="consultor"))
    .forEach(u=>(u.polos||[]).forEach(p=>{
      if (!map[p.codigo]) map[p.codigo] = { ...p, donoId:u.id, donoNome:u.nome, donoRole:u.role };
    }));
...
function RoleBadge({ role }) {
  const map = { admin:{bg:"#1a1a3e",tx:"#fff",label:"Super Admin"}, gestor:{bg:"#e0e7ff",tx:"#3730a3",label:"Gestor"}, consultor:{bg:"#fef9c3",tx:"#854d0e",label:"Consultor"}, distribuidor:{bg:"#d4edda",tx:"#155724",label:"Distribuidor"} };
  const r = map[role]||{bg:"#eee",tx:"#333",label:role};
  return <span style={{ background:r.bg, color:r.tx, fontSize:11, padding:"2px 8px", borderRadius:10, fontWeight:600 }}>{r.label}</span>;
}
function LogoMark() {
  return (
...
// ── Polo Form (reutilizável) ──
function PolosForm({ polos, setPolos, allPolos=[] }) {
  function add() { setPolos([...polos,{ cidade:"", estado:"", codigo:"", isNew:true }]); }
  function remove(i) { if(polos.length===1)return; setPolos(polos.filter((_,idx)=>idx!==i)); }
  function update(i,field,val) { setPolos(polos.map((p,idx)=>idx===i?{...p,[field]:val}:p)); }
  function selectExisting(i, codigo) {
    const found = allPolos.find(p=>p.codigo===codigo);
    if (found) setPolos(polos.map((p,idx)=>idx===i?{...found,isNew:false}:p));
  }
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <p style={{ margin:0, fontSize:13, fontWeight:500, color:"#555" }}>Polos de atuação</p>
        <button onClick={add} style={{ background:"#00e07a18", border:`1px solid ${C.green}55`, borderRadius:6, padding:"4px 12px", fontSize:12, cursor:"pointer", color:"#0a6640", fontWeight:500 }}>+ Polo</button>
      </div>
      {allPolos.length>0&&<div style={{ background:"#eeeeff", border:`1px solid ${C.dark}22`, borderRadius:8, padding:"10px 12px" }}>
        <p style={{ margin:"0 0 6px", fontSize:12, fontWeight:500, color:C.dark }}>Polos já existentes no sistema</p>
        <p style={{ margin:0, fontSize:11, color:"#666" }}>Você pode selecionar um polo existente abaixo ou criar um novo. Para atuar em um polo existente, precisará ser aprovado pelo responsável.</p>
      </div>}
      {polos.map((p,i)=>(
        <div key={i} style={{ background:"#f8f8f8", border:"1px solid #e0e0e0", borderRadius:10, padding:14 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
            ...
            {polos.length>1&&<button onClick={()=>remove(i)} style={{ background:"none", border:"none", cursor:"pointer", color:"#dc3545", fontSize:18, padding:0 }}>✕</button>}
          </div>
          {allPolos.length>0&&<div style={{ marginBottom:10 }}>
            <label style={lStyle}>Selecionar polo existente</label>
            <select value={p.codigo||""} onChange={e=>e.target.value?selectExisting(i,e.target.value):setPolos(polos.map((pl,idx)=>idx===i?{cidade:"",estado:"",codigo:"",isNew:true}:pl))} style={iStyle}>
              <option value="">— Criar novo polo —</option>
              {allPolos.map(ap=><option key={ap.codigo} value={ap.codigo}>{ap.cidade}/{ap.estado} ({ap.codigo}) — {ap.donoNome}</option>)}
            </select>
          </div>}
          {(!p.codigo||p.isNew)&&<div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            <Inp label="Cidade *" value={p.cidade} onChange={e=>update(i,"cidade",e.target.value)}/>
            <div><label style={lStyle}>Estado *</label>
              <select value={p.estado} onChange={e=>update(i,"estado",e.target.value)} style={iStyle}>
                <option value="">Selecione...</option>
                {ESTADOS.map(uf=><option key={uf}>{uf}</option>)}
              </select>
            </div>
            <Inp label="Código do polo *" value={p.codigo} onChange={e=>update(i,"codigo",e.target.value)} placeholder="Ex: SOC-001"/>
          </div>}
          {p.codigo&&!p.isNew&&<div style={{ background:"#fff3cd", border:"1px solid #ffc107", borderRadius:6, padding:"8px 10px" }}>
            <p style={{ margin:0, fontSize:12, color:"#856404" }}>⏳ Polo existente — você precisará de aprovação do responsável para atuar nele.</p>
          </div>}
        </div>
      ))}
    </div>
...
function ModalDadosCandidato({ lead, onSave, onClose }) {
  ...
  function salvar() {
    if(!["nomeCompleto","cpf","rg","nascimento","email","telefone","endereco","estadoCivil"].every(k=>f[k]?.trim())){alert("Preencha todos os campos.");return;}
    onSave(f);
  }
  return (
...
function useStorage() {
  ...
  useEffect(()=>{loadData().then(d=>setData({users:[],leads:[],...d}));},[]);
...
// ── Painel de leads genérico (para gestor/consultor) ──
function LeadsList({leads,mover,showMsg}){
  return(<>
    {leads.length===0&&<p style={{color:"#aaa",fontSize:13}}>Nenhum lead ainda.</p>}
    {leads.map(l=>(
      <div key={l.id} style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:10,padding:"12px 16px",marginBottom:6}}>
        <div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
          <div>
            ...
            <p style={{margin:0,fontSize:12,color:"#666"}}>{l.curso} · {l.modalidade} · Distribuidor: {l.parceiroNome}</p>
          </div>
...
// ── Distribuidores panel (para gestor/consultor) ──
function DistribuidoresPanel({meusPolo,meusDistribuidores,leads,onAprovar,session}){
  ...
  return(<>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
          {meusPolo.filter(c=>u.statusPolos?.[c]==="pendente").map(c=>{
            ...
            return polo?(<div key={c} style={{background:"#fff",borderRadius:8,padding:"8px 12px",border:"1px solid #e0e0e0",display:"flex",alignItems:"center",gap:10}}>
              ...
              <button onClick={()=>onAprovar(u.id,c,"aprovado")} style={{background:"#155724",color:"#fff",border:"none",borderRadius:5,padding:"3px 10px",fontSize:11,cursor:"pointer"}}>Aprovar</button>
              <button onClick={()=>onAprovar(u.id,c,"rejeitado")} style={{background:"#721c24",color:"#fff",border:"none",borderRadius:5,padding:"3px 10px",fontSize:11,cursor:"pointer"}}>Rejeitar</button>
            </div>):null;
          })}
        </div>
      </div>
    ...
    {meusDistribuidores.map(u=>(
      <div key={u.id} style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:10,padding:"12px 16px",marginBottom:6}}>
        ...
        <p style={{margin:"0 0 6px",fontSize:12,color:"#666"}}>{u.email} · {leads.filter(l=>l.parceiroId===u.id).length} lead(s)</p>
...
function RegisterGestorScreen({onBack,onDone,showMsg,allPolos}){
  ...
  const [polos,setPolos]=useState([{cidade:"",estado:"",codigo:"",isNew:true}]);
      ...
      <div style={{width:"100%",maxWidth:480,background:"#fff",border:"1px solid #e0e0e0",borderRadius:16,padding:"28px 24px"}}>
        ...
        {step===1&&<div style={{display:"flex",flexDirection:"column",gap:12,marginTop:16}}>
          ...
          <button onClick={()=>{
            ...
            if(!email.endsWith("@fecaf.com.br")){showMsg("O e-mail deve ter extensão @fecaf.com.br","error");return;}
            setStep(2);
          }} style={{...btnP,marginTop:4}}>Continuar</button>
        </div>}
        {step===2&&<div style={{marginTop:16}}>
          <PolosForm polos={polos} setPolos={setPolos} allPolos={allPolos}/>
          <button onClick={()=>{if(!polos.every(p=>p.cidade&&p.estado&&p.codigo)){showMsg("Preencha todos os campos dos polos.","error");return;}setStep(3);}} style={{...btnP,marginTop:14,width:"100%"}}>Continuar</button>
        </div>}
...
function RegisterConsultorScreen({onBack,onDone,showMsg,allPolos}){
  const [step,setStep]=useState(1);
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",background:"#f2f2f7",padding:20}}>
      <div style={{width:"100%",maxWidth:480,background:"#fff",border:"1px solid #e0e0e0",borderRadius:16,padding:"28px 24px"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}>
          <button onClick={()=>step>1?setStep(step-1):onBack()} style={{background:"none",border:"none",cursor:"pointer",color:"#666",fontSize:22,padding:0}}>←</button>
          <div style={{flex:1}}><h2 style={{margin:0,fontSize:16,fontWeight:500}}>Cadastro de Consultor de Parceiros</h2>{bar}<p style={{margin:"4px 0 0",fontSize:11,color:"#999"}}>Etapa {step} de 3</p></div>
        </div>
        {step===1&&<div style={{display:"flex",flexDirection:"column",gap:12,marginTop:16}}>
          <p style={{margin:0,fontSize:13,fontWeight:500,color:"#555"}}>Dados pessoais</p>
          <Inp label="Nome completo *" value={nome} onChange={e=>setNome(e.target.value)}/>
        ...
        {step===3&&<div style={{display:"flex",flexDirection:"column",gap:12,marginTop:16}}>
          <p style={{margin:0,fontSize:13,fontWeight:500,color:"#555"}}>Senha e contrato</p>
          ...
          <button onClick={()=>{
            ...
            onDone({nome,email,telefone,cpfCnpj,cidade,estado,senha,polos,role:"consultor",status:"pendente",
              statusPolosConsultor: polos.reduce((acc,p)=>({...acc,[p.codigo]:p.isNew?"aprovado":"pendente"}),{}),
              vinculacoesGestor:{},criadoEm:new Date().toLocaleDateString("pt-BR")});
          }} style={{...btnP,marginTop:4}}>Enviar cadastro</button>
        </div>}
      </div>
    </div>
...
function RegisterScreen({onBack,onDone,showMsg,allPolos}){
  const [step,setStep]=useState(1);
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",background:"#f2f2f7",padding:20}}>
      <div style={{width:"100%",maxWidth:440,background:"#fff",border:"1px solid #e0e0e0",borderRadius:16,padding:"28px 24px"}}>
        ...
        {step===2&&<div style={{display:"flex",flexDirection:"column",gap:12,marginTop:16}}>
          ...
          {allPolos.length===0?<p style={{fontSize:13,color:"#aaa"}}>Nenhum polo disponível.</p>
          :allPolos.map(polo=>{
            ...
            return(<label key={polo.codigo} style={{display:"flex",alignItems:"center",gap:12,border:`1.5px solid ${sel?C.dark:"#ccc"}`,borderRadius:10,padding:"12px 14px",cursor:"pointer",background:sel?"#eeeeff":"#fff"}}>
              ...
              <div><p style={{margin:0,fontSize:13,fontWeight:sel?600:400}}>{polo.cidade} — {polo.estado}</p><p style={{margin:0,fontSize:11,color:"#888"}}>Código: {polo.codigo} · Responsável: {polo.donoNome} ({polo.donoRole==="gestor"?"Gestor":"Consultor"})</p></div>
            </label>);
          })}
...
export default function App(){
  ...
  const [consultorTab,setConsultorTab]=useState("distribuidores");
  ...
  const moverHook=useMoverLead(data||{users:[],leads:[]},upd);
  ...
  const allPolos=getAllPolos(data.users||[]);
  ...
  function handleRegisterUser(reg){
    ...
    const msgs={gestor:"Cadastro enviado! Aguarde aprovação do Super Admin.",consultor:"Cadastro enviado! Aguarde aprovação do Super Admin.",distribuidor:"Cadastro enviado! Aguarde aprovação do responsável pelo polo."};
    showMsg(msgs[reg.role]||"Cadastro enviado!");setScreen("login");
  }
  ...
  function aprovarNoMeuPolo(userId,poloCode,status,owners){
    const nd={...data,users:(data.users||[]).map(u=>{
      if(u.id!==userId)return u;
      const newSP={...u.statusPolos,[poloCode]:status};
      return {...u,statusPolos:newSP,status:Object.values(newSP).some(s=>s==="aprovado")?"aprovado":"pendente"};
    })};
    upd(nd);showMsg(status==="aprovado"?"Distribuidor aprovado neste polo!":"Rejeitado neste polo.");
  }

  // Convite gestor→consultor
  function convidarConsultor(gestorId,consultorId){
    const nd={...data,users:(data.users||[]).map(u=>{
      if(u.id!==consultorId)return u;
      const vg={...u.vinculacoesGestor,[gestorId]:{status:"pendente_consultor",criadoEm:new Date().toLocaleDateString("pt-BR")}};
      return {...u,vinculacoesGestor:vg};
    })};
    upd(nd);showMsg("Convite enviado ao Consultor!");
  }
  function responderConvite(consultorId,gestorId,aceitar){
    const nd={...data,users:(data.users||[]).map(u=>{
      if(u.id!==consultorId)return u;
      const vg={...u.vinculacoesGestor,[gestorId]:{...u.vinculacoesGestor[gestorId],status:aceitar?"pendente_admin":"recusado"}};
      return {...u,vinculacoesGestor:vg};
    })};
    upd(nd);showMsg(aceitar?"Convite aceito! Aguarda aprovação do Super Admin.":"Convite recusado.");
  }
  function aprovarVinculacao(consultorId,gestorId,aprovar){
    const nd={...data,users:(data.users||[]).map(u=>{
      if(u.id!==consultorId)return u;
      const vg={...u.vinculacoesGestor,[gestorId]:{...u.vinculacoesGestor[gestorId],status:aprovar?"aprovado":"rejeitado"}};
      return {...u,vinculacoesGestor:vg};
    })};
    upd(nd);showMsg(aprovar?"Vinculação aprovada!":"Vinculação rejeitada.");
  }
  ...
  if(screen==="login") return(<><MsgBanner msg={msg}/><LoginScreen onLogin={handleLogin} onRegisterDist={()=>setScreen("register-dist")} onRegisterGestor={()=>setScreen("register-gestor")} onRegisterConsultor={()=>setScreen("register-consultor")}/></>);
  if(screen==="register-dist") return(<><MsgBanner msg={msg}/><RegisterScreen onBack={()=>setScreen("login")} onDone={handleRegisterUser} showMsg={showMsg} allPolos={allPolos}/></>);
  if(screen==="register-gestor") return(<><MsgBanner msg={msg}/><RegisterGestorScreen onBack={()=>setScreen("login")} onDone={handleRegisterUser} showMsg={showMsg} allPolos={allPolos}/></>);
  if(screen==="register-consultor") return(<><MsgBanner msg={msg}/><RegisterConsultorScreen onBack={()=>setScreen("login")} onDone={handleRegisterUser} showMsg={showMsg} allPolos={allPolos}/></>);
  ...
  if(screen==="app"&&session?.role==="admin"){
    ...
    const vinculacoesPend=(data.users||[]).filter(u=>u.role==="consultor"&&Object.values(u.vinculacoesGestor||{}).some(v=>v.status==="pendente_admin"));
    const tarefas=(data.leads||[]).filter(l=>l.tarefaAdmin).length;
    const totalPend=gestoresPend.length+consultoresPend.length+distPend.length+vinculacoesPend.length;

    function UserCard({u,onAprovar,onRejeitar,showPolos=true}){
      ...
      return(<div style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:10,padding:"12px 16px",marginBottom:6}}>
        <div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
          <div>
            ...
            {showPolos&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginTop:4}}>{(u.polos||[]).map((p,i)=><span key={i} style={{background:"#eeeeff",border:`1px solid ${C.dark}22`,borderRadius:6,padding:"2px 8px",fontSize:11,color:C.dark}}>{p.cidade}/{p.estado} ({p.codigo})</span>)}</div>}
          </div>
          <div style={{display:"flex",gap:8,alignItems:"flex-start"}}>
            {u.status!=="aprovado"&&<button onClick={()=>onAprovar(u.id)} style={{background:"none",border:"1px solid #155724",borderRadius:6,padding:"5px 12px",fontSize:12,cursor:"pointer",color:"#155724"}}>Aprovar</button>}
            {u.status==="aprovado"&&<button onClick={()=>onRejeitar(u.id)} style={{background:"none",border:"1px solid #dc3545",borderRadius:6,padding:"5px 12px",fontSize:12,cursor:"pointer",color:"#dc3545"}}>Suspender</button>}
            {u.status==="rejeitado"&&<button onClick={()=>onAprovar(u.id)} style={{background:"none",border:"1px solid #155724",borderRadius:6,padding:"5px 12px",fontSize:12,cursor:"pointer",color:"#155724"}}>Reativar</button>}
          </div>
        </div>
      </div>);
    }
    ...
    return(
      <div style={{minHeight:"100vh",background:"#f2f2f7"}}>
        ...
        <TabBar tabs={[["gestores","Gestores",gestoresPend.length],["consultores","Consultores",consultoresPend.length+vinculacoesPend.length],["distribuidores","Distribuidores",distPend.length],["leads","Leads",tarefas],["comissoes","Comissões",0]]} active={adminTab} setActive={setAdminTab}/>
        <div style={{padding:20,maxWidth:900,margin:"0 auto"}}>
          ...
          {adminTab==="gestores"&&<>
            ...
            {(data.users||[]).filter(u=>u.role==="gestor").map(u=><UserCard key={u.id} u={u} onAprovar={id=>updateUserStatus(id,"aprovado")} onRejeitar={id=>updateUserStatus(id,"rejeitado")}/>)}
          </>}

          {adminTab==="consultores"&&<>
            {vinculacoesPend.length>0&&<><h3 style={{margin:"0 0 12px",fontSize:14,fontWeight:500,color:"#856404"}}>Vinculações aguardando aprovação ({vinculacoesPend.length})</h3>
            {vinculacoesPend.map(c=>{
              const vgs=Object.entries(c.vinculacoesGestor||{}).filter(([,v])=>v.status==="pendente_admin");
              return vgs.map(([gId,v])=>{
                const gestor=(data.users||[]).find(u=>u.id===parseInt(gId));
                return(<div key={`${c.id}-${gId}`} style={{background:"#fff3cd",border:"1px solid #ffc107",borderRadius:10,padding:"14px 16px",marginBottom:10}}>
                  <p style={{margin:"0 0 4px",fontSize:13,color:"#856404"}}>Consultor: <strong>{c.nome}</strong> ↔ Gestor: <strong>{gestor?.nome||gId}</strong></p>
                  <p style={{margin:"0 0 10px",fontSize:12,color:"#856404"}}>Solicitado em {v.criadoEm}</p>
                  <div style={{display:"flex",gap:8}}>
            ...
            {consultoresPend.map(c=>(
              <div key={c.id} style={{background:"#fff3cd",border:"1px solid #ffc107",borderRadius:10,padding:"14px 16px",marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                  <div><p style={{margin:"0 0 2px",fontWeight:500,fontSize:14,color:"#856404"}}>{c.nome}</p>
                    <p style={{margin:"0 0 8px",fontSize:12,color:"#856404"}}>{c.email} · {c.cidade}/{c.estado}</p>
                    <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{(c.polos||[]).map((p,i)=><span key={i} style={{background:"#fff",border:"1px solid #ffc107",borderRadius:6,padding:"2px 8px",fontSize:11,color:"#856404"}}>{p.cidade}/{p.estado} — {p.codigo}</span>)}</div>
                  </div>
                  <div style={{display:"flex",gap:8}}>
            ...
            {(data.users||[]).filter(u=>u.role==="consultor").map(u=><UserCard key={u.id} u={u} onAprovar={id=>updateUserStatus(id,"aprovado")} onRejeitar={id=>updateUserStatus(id,"rejeitado")}/>)}
          </>}
          ...
          {adminTab==="distribuidores"&&<>
            ...
            {(data.users||[]).filter(u=>u.role==="distribuidor").map(u=><UserCard key={u.id} u={u} showPolos={false} onAprovar={id=>updateUserStatus(id,"aprovado")} onRejeitar={id=>updateUserStatus(id,"rejeitado")}/>)}
          </>}
          ...
          {adminTab==="leads"&&<>
            <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:14,alignItems:"flex-end"}}>
              <div style={{flex:1,minWidth:180}}>
                ...
                <select style={{...iStyle,fontSize:13,padding:"7px 10px"}} defaultValue="todos">
                  <option value="todos">Todos</option>
                  {(data.users||[]).filter(u=>u.role==="distribuidor").map(u=><option key={u.id} value={u.id}>{u.nome}</option>)}
                </select>
              </div>
            </div>
            {(data.leads||[]).map(l=>(
              <div key={l.id} style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:10,padding:"12px 16px",marginBottom:6}}>
                <div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                  <div>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:2}}><p style={{margin:0,fontWeight:500,fontSize:14}}>{l.nome}</p><Badge status={l.status}/>{l.tarefaAdmin&&<span style={{background:"#fff3cd",color:"#856404",fontSize:10,padding:"1px 6px",borderRadius:8,fontWeight:600}}>⏳ Confirmar</span>}</div>
  ...
  if(screen==="app"&&session?.role==="gestor"){
    ...
    const consultoresDisponiveis=(data.users||[]).filter(u=>u.role==="consultor"&&u.status==="aprovado");
    const meusConsultores=consultoresDisponiveis.filter(u=>{const v=u.vinculacoesGestor?.[session.id];return v&&v.status==="aprovado";});
    const convitesPend=consultoresDisponiveis.filter(u=>{const v=u.vinculacoesGestor?.[session.id];return v&&v.status==="pendente_consultor";});
    const pendVinc=consultoresDisponiveis.filter(u=>{const v=u.vinculacoesGestor?.[session.id];return v&&v.status==="pendente_admin";});
    const pendDist=(data.users||[]).filter(u=>u.role==="distribuidor"&&u.polosSelecionados?.some(c=>meusPolo.includes(c))&&meusPolo.some(c=>u.statusPolos?.[c]==="pendente"));

    return(
      <div style={{minHeight:"100vh",background:"#f2f2f7"}}>
        <MsgBanner msg={msg}/><Modais/>
        <AppHeader session={session} logout={logout}/>
        <TabBar tabs={[["distribuidores","Distribuidores",pendDist.length],["consultores","Consultores",0],["leads","Leads",0],["meuspolos","Meus Polos",0]]} active={gestorTab} setActive={setGestorTab}/>
        <div style={{padding:16,maxWidth:860,margin:"0 auto"}}>
          {gestorTab==="meuspolos"&&<>
            <h3 style={{margin:"0 0 14px",fontSize:15,fontWeight:500}}>Meus polos</h3>
            {(session.polos||[]).map((p,i)=><div key={i} style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:10,padding:"12px 16px",marginBottom:8}}><p style={{margin:"0 0 2px",fontWeight:500,fontSize:14}}>{p.cidade} — {p.estado}</p><p style={{margin:0,fontSize:12,color:"#888"}}>Código: {p.codigo}</p></div>)}
          </>}
          {gestorTab==="distribuidores"&&<DistribuidoresPanel meusPolo={meusPolo} meusDistribuidores={meusDistribuidores} leads={data.leads||[]} onAprovar={(uid,c,st)=>aprovarNoMeuPolo(uid,c,st)} session={session}/>}
          {gestorTab==="consultores"&&<>
            <h3 style={{margin:"0 0 14px",fontSize:15,fontWeight:500}}>Consultores de Parceiros</h3>
            ...
            {pendVinc.length>0&&<div style={{background:"#e0e7ff",border:"1px solid #a5b4fc",borderRadius:8,padding:"10px 14px",marginBottom:14}}><p style={{margin:0,fontSize:13,color:"#3730a3"}}>⏳ {pendVinc.length} vinculação(ões) aguardando aprovação do Super Admin.</p></div>}
            <h4 style={{margin:"0 0 10px",fontSize:13,fontWeight:500,color:"#666"}}>Meus consultores vinculados ({meusConsultores.length})</h4>
            {meusConsultores.map(c=><div key={c.id} style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:10,padding:"12px 16px",marginBottom:6}}><p style={{margin:"0 0 2px",fontWeight:500,fontSize:14}}>{c.nome}</p><p style={{margin:0,fontSize:12,color:"#666"}}>{c.email} · {c.cidade}/{c.estado}</p></div>)}
            <h4 style={{margin:"16px 0 10px",fontSize:13,fontWeight:500,color:"#666"}}>Convidar consultor</h4>
            {consultoresDisponiveis.filter(c=>!c.vinculacoesGestor?.[session.id]).map(c=>(
              <div key={c.id} style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:10,padding:"12px 16px",marginBottom:6,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div><p style={{margin:"0 0 2px",fontWeight:500,fontSize:14}}>{c.nome}</p><p style={{margin:0,fontSize:12,color:"#666"}}>{c.email} · {c.cidade}/{c.estado}</p></div>
          ...
          {gestorTab==="leads"&&<><h3 style={{margin:"0 0 12px",fontSize:14,fontWeight:500,color:"#666"}}>Leads dos seus distribuidores ({meusLeads.length})</h3><LeadsList leads={meusLeads} mover={tentar} showMsg={showMsg}/></>}
        </div>
      </div>
    );
  }
  ...
  if(screen==="app"&&session?.role==="consultor"){
    ...
    const gestoresVinculados=Object.entries(session.vinculacoesGestor||{}).filter(([,v])=>v.status==="aprovado").map(([gId])=>(data.users||[]).find(u=>u.id===parseInt(gId))).filter(Boolean);

    return(
      <div style={{minHeight:"100vh",background:"#f2f2f7"}}>
        <MsgBanner msg={msg}/><Modais/>
        <AppHeader session={session} logout={logout}/>
        <TabBar tabs={[["distribuidores","Distribuidores",pendDist.length],["gestores","Gestores",convitesRecebidos.length],["leads","Leads",0],["meuspolos","Meus Polos",0]]} active={consultorTab} setActive={setConsultorTab}/>
        <div style={{padding:16,maxWidth:860,margin:"0 auto"}}>
          {consultorTab==="meuspolos"&&<>
            <h3 style={{margin:"0 0 14px",fontSize:15,fontWeight:500}}>Meus polos</h3>
            {(session.polos||[]).map((p,i)=><div key={i} style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:10,padding:"12px 16px",marginBottom:8}}><p style={{margin:"0 0 2px",fontWeight:500,fontSize:14}}>{p.cidade} — {p.estado}</p><p style={{margin:0,fontSize:12,color:"#888"}}>Código: {p.codigo}</p></div>)}
          </>}
          {consultorTab==="distribuidores"&&<DistribuidoresPanel meusPolo={meusPolo} meusDistribuidores={meusDistribuidores} leads={data.leads||[]} onAprovar={(uid,c,st)=>aprovarNoMeuPolo(uid,c,st)} session={session}/>}
          {consultorTab==="gestores"&&<>
            {convitesRecebidos.length>0&&<><h3 style={{margin:"0 0 12px",fontSize:14,fontWeight:500,color:"#856404"}}>Convites pendentes ({convitesRecebidos.length})</h3>
            {convitesRecebidos.map(([gId,v])=>{
              ...
              return(<div key={gId} style={{background:"#fff3cd",border:"1px solid #ffc107",borderRadius:10,padding:"14px 16px",marginBottom:10}}>
                <p style={{margin:"0 0 4px",fontWeight:500,fontSize:14,color:"#856404"}}>{gestor?.nome||"Gestor"} quer te vincular à sua rede</p>
                <p style={{margin:"0 0 10px",fontSize:12,color:"#856404"}}>{gestor?.email} · Solicitado em {v.criadoEm}</p>
                <div style={{display:"flex",gap:8}}>
            ...
            <h3 style={{margin:"16px 0 12px",fontSize:14,fontWeight:500,color:"#666"}}>Gestores vinculados ({gestoresVinculados.length})</h3>
            {gestoresVinculados.length===0&&<p style={{color:"#aaa",fontSize:13}}>Nenhum gestor vinculado ainda.</p>}
            {gestoresVinculados.map(g=><div key={g.id} style={{background:"#fff",border:"1px solid #e0e0e0",borderRadius:10,padding:"12px 16px",marginBottom:6}}><p style={{margin:"0 0 2px",fontWeight:500,fontSize:14}}>{g.nome}</p><p style={{margin:0,fontSize:12,color:"#666"}}>{g.email}</p></div>)}
          </>}
          {consultorTab==="leads"&&<><h3 style={{margin:"0 0 12px",fontSize:14,fontWeight:500,color:"#666"}}>Leads dos seus distribuidores ({meusLeads.length})</h3><LeadsList leads={meusLeads} mover={tentar} showMsg={showMsg}/></>}
        </div>
      </div>
    );
  }
